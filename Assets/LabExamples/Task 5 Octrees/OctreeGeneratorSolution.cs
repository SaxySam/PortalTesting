...
public class OctreeGeneratorSolution : MonoBehaviour
{
    ...
    public class OctTreeNode
    {
        ...
        public OctTreeNode(Rect3D boundingRect, List<GameObject> gameObjects)
        {
            ...
            if (gameObjects.Count > 1)
            {
                ...
                // Split the octree into 8 pieces.
                 // Calculate the split position in 3 Dimensions, then
                ...
                // Store the Game Objects split into the oct tree
                ...
                Vector3 split = new Vector3(boundingRect.x + (boundingRect.width / 2), boundingRect.y + (boundingRect.height / 2), boundingRect.z + (boundingRect.depth/2));
                ...
                for (int i = 0; i < gameObjects.Count; i++)
                {
                    ...
                    if (gameObjects[i].transform.position.x <= split.x && gameObjects[i].transform.position.y <= split.y && gameObjects[i].transform.position.z <= split.z)
                    {
                        // Add to Split 1
                        split1.Add(gameObjects[i]);
                    }
                    // go.x > s.x && go.y <= s.y && go.z <= s.z
                    else if (gameObjects[i].transform.position.x > split.x && gameObjects[i].transform.position.y <= split.y && gameObjects[i].transform.position.z <= split.z)
                    {
                        // Add to Split 2
                    ...
                    {
                        // Add to Split 5
                        split5.Add(gameObjects[i]);
                    }
                    // go.x > s.x && go.y <= s.y && go.z > s.z
                    else if (gameObjects[i].transform.position.x > split.x && gameObjects[i].transform.position.y <= split.y && gameObjects[i].transform.position.z > split.z)
                    {
                        // Add to Split 6
                        split6.Add(gameObjects[i]);
                    }
                    // go.x <= s.x && go.y > s.y && go.z > s.z
                    else if (gameObjects[i].transform.position.x <= split.x && gameObjects[i].transform.position.y > split.y && gameObjects[i].transform.position.z > split.z)
                    {
                        // Add to Split 7
                        split7.Add(gameObjects[i]);
                    }
                    // go.x > s.x && go.y > s.y && go.z > s.z
                    else
                    {
                        // Otherwise, add to Split 8
                        split8.Add(gameObjects[i]);
                    }
                }

                // If there are objects in split 1
                if (split1.Count > 0)
                    // Create a new QuadTreeNode for the upper left front of the split
                ...
                if (split2.Count > 0)
                    // Create a new QuadTreeNode for the upper right front of the split
                    node2 = new OctTreeNode(new Rect3D(split.x, nodeBoundingRect.y, nodeBoundingRect.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split2);

                // If there are objects in split 3
                if (split3.Count > 0)
                    // Create a new QuadTreeNode for the lower left front of the split
                    node3 = new OctTreeNode(new Rect3D(nodeBoundingRect.x, split.y, nodeBoundingRect.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split3);

                // If there are objects in split 4
                if (split4.Count > 0)
                    // Create a new QuadTreeNode for the lower right front of the split
                    node4 = new OctTreeNode(new Rect3D(split.x, split.y, nodeBoundingRect.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split4);

                // If there are objects in split 5
                if (split5.Count > 0)
                    // Create a new QuadTreeNode for the upper left back of the split
                    node5 = new OctTreeNode(new Rect3D(nodeBoundingRect.x, nodeBoundingRect.y, split.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split5);

                // If there are objects in split 6
                if (split6.Count > 0)
                    // Create a new QuadTreeNode for the upper right back of the split
                    node6 = new OctTreeNode(new Rect3D(split.x, nodeBoundingRect.y, split.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split6);

                // If there are objects in split 7
                if (split7.Count > 0)
                    // Create a new QuadTreeNode for the lower left back of the split
                    node7 = new OctTreeNode(new Rect3D(nodeBoundingRect.x, split.y, split.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split7);

                // If there are objects in split 8
                if (split8.Count > 0)
                    // Create a new QuadTreeNode for the lower right back of the split
                    node8 = new OctTreeNode(new Rect3D(split.x, split.y, split.z, nodeBoundingRect.width / 2, nodeBoundingRect.height / 2, nodeBoundingRect.depth / 2), split8);
            }

...
public class RedLightPlayerSolution : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (transform.Find("VisibilitySphere")!=null)
        {
            ...
            // Check to see this gameobject is in the FrustumCull's list of objects in view and 
            ...
            Color alpha = new Color(1, 1, 1, sphereMaterial.color.a);
            if (frustumCull.gameObjectsInFrustum.Contains(gameObject))
            {
                if (currentSpeed > 0) // Caught
                {

                    sphereMaterial.color =Color.red * alpha;
                    canMove = false;
                } 
                else // Safe
                {
                    sphereMaterial.color = Color.green * alpha;
                }
            }
             else // Not in view
            {
                sphereMaterial.color = Color.grey * alpha;
            }
